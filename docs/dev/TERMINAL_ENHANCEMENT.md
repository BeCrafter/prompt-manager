...
在 `WebSocketConnection.js` 中添加新的消息类型：
...
async handleCustomCommand(message) {
  try {
    const result = await terminalService.customFunction(message);
    this.send('terminal.custom.result', result);
  } catch (error) {
    this.sendError('Custom command failed', error.message);
  }
}
```
...
handleMessage(message) {
  switch (message.type) {
    // ... 现有处理
    case 'terminal.custom.result':
      this.handleCustomResult(message);
      break;
  }
}
```
...
const terminalService = new TerminalService({
  ...
  timeout: 300000,        // 会话超时时间（毫秒）
  workingDirectory: os.homedir(), // 默认工作目录
  size: { cols: 80, rows: 24 }   // 默认终端大小
});
```
...
```javascript
// 在浏览器控制台中监控消息
window.terminalComponent.websocket.addEventListener('message', (event) => {
...
console.log('Terminal service status:', status);

const wsStatus = webSocketService.getStatus();
console.log('WebSocket service status:', wsStatus);
```
