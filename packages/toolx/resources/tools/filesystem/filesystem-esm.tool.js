/**
 * Filesystem Tool - PromptManager 体系的文件系统基础设施 (ESM 版本)
 * 
 * 战略意义：
 * 
 * 1. 架构隔离性
...
// 模拟 importx 函数，用于在示例环境中演示
// 在实际的 ToolSandbox 环境中，importx 会由沙箱提供
async function importx(moduleName) {
 ...
 * 获取业务错误定义
 */
export function getBusinessErrors() {
  return [
    {
...
async function getMCPInstance() {
  if (typeof globalThis._mcpInstance !== 'undefined') {
    return globalThis._mcpInstance;
  }
  ...
  globalThis._mcpInstance = mcpModule;
  globalThis._allowedDirectories = allowedDirectories;
  
  // 记录日志
  const api = {
    logger: {
      info: (msg, data) => console.log('ToolX Log:', msg, data)
    }
  };
  api.logger.info('Filesystem initialized', { 
    allowedDirectories: globalThis._allowedDirectories 
  });
...
function getAllowedDirectories() {
  ...
  const env = {
    get: (key) => {
      if (key === 'ALLOWED_DIRECTORIES') {
        // 这里可以使用环境变量，但为了演示，我们返回默认值
        return '["./test-real-output", "/tmp"]';
      }
      return null;
    }
  };
  ...
  try {
    let configStr = env.get('ALLOWED_DIRECTORIES');
    if (configStr) {
    ...
    console.log('Failed to parse ALLOWED_DIRECTORIES', { error: error.message });
  }
...
// 辅助函数 - PromptManager特定的路径处理
function resolvePromptManagerPath(inputPath) {
  // 模拟 api
  const api = {
    logger: {
      warn: (msg, data) => console.log('ToolX Warn:', msg, data)
    }
  };
  
  // 获取允许的目录列表
  const allowedDirs = globalThis._allowedDirectories || getAllowedDirectories();
...
export async function execute(params) {
  // 模拟 this.api，实际 ToolSandbox 会提供
  const api = {
    logger: {
      info: (msg, data) => console.log('ToolX Log:', msg, data),
      warn: (msg, data) => console.log('ToolX Warn:', msg, data),
      error: (msg, data) => console.log('ToolX Error:', msg, data)
    }
  };
  ...
  try {
    ...
    // 路径参数转换
    if (params.path) {
      mcpParams.path = resolvePromptManagerPath(params.path);
    }
    
    if (params.paths) {
      mcpParams.paths = params.paths.map(p => resolvePromptManagerPath(p));
    }
    
    if (params.source) {
      mcpParams.source = resolvePromptManagerPath(params.source);
    }
    
    if (params.destination) {
      mcpParams.destination = resolvePromptManagerPath(params.destination);
    }
    ...
    switch (params.method) {
      ...
      case 'create_directory': {
        await fs.mkdir(mcpParams.path, { recursive: true });
      ...
      case 'list_directory_with_sizes': {
        const entries = await fs.readdir(mcpParams.path, { withFileTypes: true });
      ...
      case 'move_file': {
        await fs.rename(mcpParams.source, mcpParams.destination);
        result = {
          from: params.source,
      ...
      case 'search_files': {
        ...
        // 转换为相对路径（相对于第一个允许的目录）
        const baseDir = getAllowedDirectories()[0];
