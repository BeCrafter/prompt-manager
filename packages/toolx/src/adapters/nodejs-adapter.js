/**
 * Node.js运行时适配器 - 为Node.js环境提供沙箱支持
 * 使用Child Process实现进程隔离
 */
...
class NodeJSRuntimeProvider {
  ...
  async initialize() {
    ...
    this.processPool = new NodeJSProcessPool(this, {
  ...
  async executeCommand(command, args, options = {}) {
    ...
    try {
      ...
      // 设置超时
      timeoutTimer = this.resourceLimiter.createTimeout(() => {
        throw new Error(`Command execution timeout: ${command}`);
      }, options.timeout || this.limits.maxExecutionTime);
      
      // 执行命令
      const result = await this.executeInChildProcess(worker, command, args, options);
   ...
   * @returns {Promise<object>} 执行结果
   */
  async executeInChildProcess(worker, command, args, options) {
 ...
 * Node.js进程池实现
 */
class NodeJSProcessPool extends BaseProcessPool {
  ...
  async doCreateWorker(workerId) {
    ...
    // 创建测试进程
    worker.process = spawn('node', ['--version'], {
